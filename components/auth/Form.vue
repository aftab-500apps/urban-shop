...
<template>
  <form class="space-y-6" @submit.prevent="handleSubmit">
    <div v-for="(field, index) in formFields.inputFields" :key="index">
      <label :for="field.name" class="block text-sm   font-medium leading-6 text-slate-100">
        Enter {{ field.name }}
      </label>
      <div class="mt-2">
        <input :id="field.name" :name="field.name" :placeholder="`Enter ${field.name}`"
          v-model="inputOBJ[field.name].value" :type="field.type" :autocomplete="field.name"
